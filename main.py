...
parser = argparse.ArgumentParser(
    description="A Command-Line To-Do app, written in Python 🐍."
)
...
class menu:
    def main():
        ...
        option = inquirer.select(
            message="What would you like to do?", choices=options
        ).execute()
...
class tasks:
    def show():  # Request a list of tasks from database.py, then display them in a table
        ...
        print(
            tabulate.tabulate(
                tasks, headers=["Title", "Description", "Created/Edited"]
            ),
        )
        input("\nPress Enter.")
        menu.main()
    ...
    def edit():  # Lookup existing titles in the database and edit them
        ...
        description = inquirer.text(message="New Description:").execute()
        if description:
    ...
    def search():  # Internal search function for delete and edit
        ...
        if not tasks:
            ...
            task = inquirer.fuzzy(
                message="Select a task to Delete:", choices=tasks
            ).execute()
    ...
    def clear_all():  # Hidden option for testing/debug purposes
        ...
        option = inquirer.confirm(message="ARE YOU SURE?").execute()
        if option:
            db.drop()
            input("\nTable Dropped.")
        menu.main()
